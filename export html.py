...
===========================================
Run this after fetch_scores.py to regenerate the website HTML.

Usage:
    python export_html.py                    # Generates leaderboard.html
    python export_html.py --out my_page.html # Custom output path
"""
...
def generate_html(excel_path, out_path):
    # Load logo (place IMG_8424.jpg in same folder as this script)
    import base64, os
...
<style>
:root{{--bg:#0d1117;--surface:#161b22;--border:#30363d;--accent:#e94560;--gold:#f5a623;--text:#e6edf3;--muted:#8b949e;--green:#3fb950;}}
*{{box-sizing:border-box;margin:0;padding:0;}}
body{{background:var(--bg);color:var(--text);font-family:'Segoe UI',system-ui,sans-serif;}}
.hero{{background:linear-gradient(135deg,#1a1a2e,#16213e,#0f3460);padding:48px 24px 36px;text-align:center;border-bottom:2px solid var(--accent);}}
.hero h1{{font-size:clamp(1.6rem,4vw,2.6rem);font-weight:800;}}
.hero h1 span{{color:var(--accent);}}
.hero p{{color:var(--muted);margin-top:8px;font-size:.95rem;}}
.stats-bar{{display:flex;justify-content:center;gap:32px;flex-wrap:wrap;padding:20px 24px;background:var(--surface);border-bottom:1px solid var(--border);}}
...
thead th{{background:#1a1a2e;color:var(--muted);font-size:.75rem;text-transform:uppercase;letter-spacing:1px;padding:12px 16px;text-align:center;white-space:nowrap;}}
...
.b48{{background:#e94560;}}.b15{{background:#2980b9;}}.b12{{background:#27ae60;}}
...
@media(max-width:600px){{td{{padding:10px 8px;font-size:.8rem;}}}}
</style></head><body>
<div class="hero" style="background:linear-gradient(135deg,#1a0000,#2d0000,#1a0a00);border-bottom:3px solid #c0392b;">
  <div style="display:flex;align-items:center;justify-content:center;gap:28px;padding:28px 24px;flex-wrap:wrap;">
    {f'<img src="{logo_tag}" style="width:110px;height:110px;object-fit:contain;filter:drop-shadow(0 4px 16px rgba(192,57,43,0.5));" alt="Logo">' if logo_tag else ""}
    <div>
      <div style="font-size:.72rem;letter-spacing:3px;text-transform:uppercase;color:#f5a623;margin-bottom:4px;">⚽ Official Leaderboard</div>
      <h1 style="font-size:clamp(1.8rem,4vw,2.8rem);font-weight:900;line-height:1.05;">
        <span style="background:linear-gradient(135deg,#8B0000,#c0392b,#e74c3c);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;">ALSALEH</span>
        <span style="color:#fff;display:block;font-size:0.55em;letter-spacing:2px;text-transform:uppercase;font-weight:700;margin-top:2px;">World Cup Tournament</span>
      </h1>
      <div style="color:#8b949e;margin-top:8px;font-size:.9rem;">Group stage predictions — updated after each match</div>
    </div>
  </div>
</div>
<div class="stats-bar">
...
<div class="content">
  <div id="leaderboard" class="panel active">
    <div class="points-key">
      <span style="font-size:.8rem;color:var(--muted)">Points system:</span>
...
<footer>Last updated: {timestamp} &nbsp;·&nbsp; ⚽ FIFA World Cup 2026</footer>
<script>
...
function filterTable(q){{
  ...
  document.querySelectorAll('#lbBody tr').forEach(r=>{{
    ...
    with open(out_path, 'w', encoding='utf-8') as f:
        f.write(html)
    print(f"✅ HTML leaderboard saved → {out_path}")
